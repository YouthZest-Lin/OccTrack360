...
class Panoptic4DEval:
  ...
  def addBatchSemIoU(self, x_sem, y_sem):
    ...
    # make confusion matrix (cols = gt, rows = pred)
    np.add.at(self.px_iou_conf_matrix, tuple(idxs.astype(np.int16)), 1)
...
class OccPanoptic4DEval(Panoptic4DEval):
  ...
  def addBatch(self, seq, x_sem, x_inst, y_sem, y_inst, interested_mask):  # x=preds, y=targets
    ''' IMPORTANT: Inputs must be batched. Either [N,H,W], or [N, P]
    '''
    #interested_mask_x = x_sem != 15
    #interested_mask_y = y_sem != 15
    #interested_mask = interested_mask_x | interested_mask_y
    #interested_mask = np.ones((np.shape(interested_mask)), dtype=bool)#YzL

    x_sem = x_sem[interested_mask]#YzL
    x_inst = x_inst[interested_mask]#YzL
