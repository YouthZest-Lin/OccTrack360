...
class VoxelAttentionAggregation(nn.Module):
    ...
    def forward(self, voxel_feat, coords_list, down_sampling_ratio=0.5):
        # for the shape of offsets_prod: [B, X, Y, Z]
        B, C, X, Y, Z = voxel_feat.shape
        voxel_feat_ds = voxel_feat
        #voxel_feat_ds = F.interpolate(voxel_feat, scale_factor=down_sampling_ratio, mode='trilinear', align_corners=self.align_corners) # here the voxel_feat has been downsampled with scale = 0.25, may need fixed
        ...
        for grid in coords_list:
            # may need fixed
            # [1,x,y,z,3] -> [1,x_ds,y_ds,z_ds,3]
            grid = grid.permute(0, 4, 1, 2, 3)  # [B, X, Y, Z, 3] -> [B, 3, X, Y, Z]


            #grid = F.avg_pool3d(grid, kernel_size=(2, 1, 2))  # [B, 3, X, Y, Z] -> [1, 3, X_ds, Y, Z_ds]
            grid = F.avg_pool3d(grid, kernel_size=(2, 1, 1))  # [B, 3, X, Y, Z] -> [1, 3, X_ds, Y, Z]

            grid = F.avg_pool3d(grid, kernel_size=(1, 2, 1))  # [1, 3, X_ds, Y, Z_ds] -> [1, 3, X_ds, Y_ds, Z_ds]


            grid = grid.permute(0, 2, 3, 4, 1)  # [1, X_ds, Y_ds, Z_ds, 3]
            if grid.shape[-1] != 3:
                grid = grid.expand(-1, -1, -1, -1, 3)
            ...
            #neighbor_feats.append(voxel_feat_ds)

        neighbor_feats = torch.stack(neighbor_feats, dim=1)
        # here I may change the Q/V , follow is the original code of the voxel aggregator of the Voxdet
        ...
        B_dir, C, D, H, W = B * self.num_directions, C, X_ds, Y_ds, Z_ds
        neighbor_feats_flat = neighbor_feats.view(B_dir, C, X_ds, Y_ds, Z_ds)
        query = self.query_conv(neighbor_feats)  # [B, C, X_ds, Y_ds, Z_ds]
        query = self.query_norm(query)
        ...
        #print(f'q_shape:{query.shape},v_shape:{values.shape},k_shape:{keys.shape},a_n_shape:{aggregated_neighbors.shape}')
        ...
        out = self.out_conv(aggregated_neighbors_up) # since the offsets size = 2*voxel_feat size, could not be plus together, may need fixed
