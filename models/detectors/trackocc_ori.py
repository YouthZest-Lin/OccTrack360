...
class TrackOcc_ori(MVXTwoStageDetector):
    def __init__(self,
                 embed_dims=256,
                 num_query=300,
                 num_classes=16,
                 ...
                 loss_cfg=None,
                **kwargs):
        super(TrackOcc_ori, self).__init__(**kwargs)
    ...
    def extract_feat(self, img, img_metas=None):
        ...
        cam_params = img[1:8] # rot, tran, intrin, post_rot, post_tran, bda
        ...
        volume_feats, depth = self.img_view_transformer([img_feats[2]] + cam_params[:] + [mlp_input]) # 
    ...
    def forward_train(self, 
        ...
        for i in range(num_frame): 
            ...
            cam_params = img_inputs[i]
            cam_params = cam_params[:4] + cam_params[5:]
            frame_res, not_track_loss = self._forward_single(track_instances, cam_params, curr2next_ego_list[i], gt_depth[i], voxel_semantics[i],\
    ...
    def ref_pts_align(self, ref_pts, ref_pts_and_velo, curr2next_ego):
        ...
        aligned_ref_pts = curr2next_ego[None, ...].to(torch.float32) @ aligned_ref_pts.to(torch.float32).unsqueeze(-1)
