...
class MeiLSSViewTransformer(BaseModule):
    ...
    def __init__(
        ...
        super(MeiLSSViewTransformer, self).__init__()
    ...
    def fast_atan2(self, y, x, r):
        # 如果外部已经计算过 r = sqrt(x^2 + y^2)，可以直接传入以节省计算
        if r is None:
            r = torch.sqrt(x**2 + y**2 + 1e-8)
        
        # phi = arccos(x/r)，此时 phi 范围是 [0, pi]
        phi = torch.acos(x / r)
        
        # 利用 y 的正负号将 phi 扩展到 (-pi, pi]
        # 如果 y < 0, 则 phi = -phi
        phi = torch.where(y >= 0, phi, -phi)
        
        return phi
    def get_lidar_coor(self, rots, trans, cam2imgs, cal, post_rots, post_trans,
                       bda):
        ...
        points = torch.inverse(cam2imgs).view(B, N, 1, 1, 1, 3, 3).matmul(points)
        x_u = points[..., 0, :]
        y_u = points[..., 1, :]
        
        # 提取视锥中的深度 d (通常存储在 z 通道)
        depth = points[..., 2, :].clone() 

        r = torch.sqrt(x_u**2 + y_u**2)
        # 避免 xi^2 = 1 导致的除零
        r_max = torch.sqrt(torch.abs(1.0 / (xi ** 2 - 1.0 + 1e-6)))
        r = torch.min(r, r_max)
        phi = torch.atan2(y_u, x_u)
        
        # 鱼眼模型下的 theta 计算
        val = 1 + r**2 * (1 - xi**2)
        # 确保 val 不为负
        theta = torch.acos((-r**2 * xi + torch.sqrt(torch.clamp(val, min=0))) / (1 + r**2))

        # 统一构建单位球面坐标并乘上深度
        # xi == 0 时退化为标准针孔，theta 会自动匹配
        mask_xi = (xi != 0)
        
        # 核心修正：
        # 如果 xi=0，则直接用原始反投影得到的点；
        # 如果 xi!=0，则使用球面模型重新分布点。
        new_x = torch.sin(theta) * torch.cos(phi) * depth
        new_y = torch.sin(theta) * torch.sin(phi) * depth
        new_z = torch.cos(theta) * depth
        
        points[..., 0, :] = torch.where(mask_xi, new_x, points[..., 0, :])
        points[..., 1, :] = torch.where(mask_xi, new_y, points[..., 1, :])
        points[..., 2, :] = torch.where(mask_xi, new_z, points[..., 2, :])
    ...
    def get_mlp_input(self, rot, tran, intrin, cal, post_rot, post_tran, bda):
        return None
...
class ASPP(nn.Module):
    ...
    def __init__(self, inplanes, mid_channels=256, BatchNorm=nn.BatchNorm2d):
        ...
        self.global_avg_pool = nn.Sequential(
            ...
            BatchNorm(mid_channels), # may need fixed
            #nn.GroupNorm(24, mid_channels),
            nn.ReLU(),
        )
...
class DepthNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.ln = nn.LayerNorm(32) # may need fixed
        #self.bn = nn.BatchNorm2d(27)
        self.depth_mlp = Mlp(32, mid_channels, mid_channels)
        self.depth_se = SELayer(mid_channels)  # NOTE: add camera-aware
        self.context_mlp = Mlp(32, mid_channels, mid_channels)
    ...
    def forward(self, x, mlp_input, stereo_metas=None):
        #mlp_input = self.ln(mlp_input.reshape(-1, mlp_input.shape[-1]))
        mlp_input = self.ln(mlp_input.reshape(-1, mlp_input.shape[-1]))
...
@NECKS.register_module()
class MeiLSSViewTransformerBEVDepth(MeiLSSViewTransformer):

    def __init__(self, loss_depth_weight=3.0, depthnet_cfg=dict(), **kwargs):
        super(MeiLSSViewTransformerBEVDepth, self).__init__(**kwargs)
    ...
    def get_depth_loss(self, depth_labels, depth_preds):
        ...
        #depth_labels = torch.minimum(depth_labels, torch.tensor(1.0, device=depth_labels.device))

        #depth_labels = torch.maximum(depth_labels, torch.tensor(0.0, device=depth_labels.device))

        #depth_preds = torch.minimum(depth_preds, torch.tensor(1.0, device=depth_preds.device))

        #depth_preds = torch.maximum(depth_preds, torch.tensor(0.0, device=depth_preds.device))
        #depth_labels = depth_labels.softmax()
        #depth_preds = depth_preds / depth_labels.max()
        #depth_labels = F.softmax(depth_labels, dim=0)
        #depth_preds = F.softmax(depth_preds, dim=0)
        #print(f'dp_min{depth_preds.min()}')
        #print(f'dp_max{depth_preds.max()}')
        #print(f'dl_min{depth_labels.min()}')
        #print(f'dl_max{depth_labels.max()}')
    ...
    def forward(self, input, stereo_metas=None):
        (x, rots, trans, intrins, cal, post_rots, post_trans, bda,
         mlp_input) = input[:9]
...
@NECKS.register_module()
class MeiLSSViewTransformerBEVStereo(MeiLSSViewTransformerBEVDepth):

    def __init__(self,  **kwargs):
        super(MeiLSSViewTransformerBEVStereo, self).__init__(**kwargs)
