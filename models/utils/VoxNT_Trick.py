...
def compute_all_direction_distances(gt_occ):
    ...
    dist_x_pos = run_length_along_dim(gt_occ, 0, 'positive')
    dist_x_neg = run_length_along_dim(gt_occ, 0, 'negative')
    dist_y_pos = run_length_along_dim(gt_occ, 1, 'positive')
    dist_y_neg = run_length_along_dim(gt_occ, 1, 'negative')
    dist_z_pos = run_length_along_dim(gt_occ, 2, 'positive')
    dist_z_neg = run_length_along_dim(gt_occ, 2, 'negative')
    
    distances = torch.stack([dist_x_pos, dist_x_neg, dist_y_pos, dist_y_neg, dist_z_pos, dist_z_neg], dim=0)
    return distances
