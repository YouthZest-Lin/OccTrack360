...
class HungarianMatcher(BaseModule):
    ...
    def forward(self, mask_pred, class_pred, mask_gt, class_gt, mask_camera):
        ...
        indices = []
        assert torch.isnan(mask_pred).any() == False ,"nan in Hungarian mask_pred"
        assert torch.isnan(mask_gt).any() == False ,"nan in Hungarian mask_gt"
        assert torch.isnan(class_pred).any() == False ,"nan in Hungarian class_pred"
        #assert torch.isnan(class_gt).any() == False ,"nan in Hungarian class_gt"
        ...
        for b in range(bs):
            ...
            out_mask = mask_pred[b]  # [num_queries, H_pred, W_pred]ma
            ...
            assert torch.isnan(C).any() == False ,"nan in Hungarian C"
            assert not torch.isinf(C).any(), "inf in Hungarian C"
