...
def random_drop_tracks(active_track_instances: Instances, drop_probability: float) -> Instances:
    '''
    if (torch.rand_like(active_track_instances.scores) > drop_probability).shape[0] == 0:
        return active_track_instances
    '''
    #if drop_probability > 0:
    '''
    if drop_probability > 0 and len(active_track_instances) > 0: # here need to be fixed drop_probability > 0 and len(active_track_instances) > 0
